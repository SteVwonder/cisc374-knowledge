...
class EndScene(spyral.Scene):
	...
	title_text = extras.Text("High Scores", (90, 1000), (WIDTH/2, 125), layer = 'top', font_size = 36, font_color = (255, 255, 255))

	self.scores_list = self.get_scores(self.filename)
	scorelist_text = []
	for score in xrange(len(self.scores_list)):
	    new_string = '{0}. {1}'.format(score+1, self.scores_list[1])
	    scorelist_text = extras.Text(new_string, (300, 50), (50, 300+(score*75)), anchor = 'topleft', layer = 'top', font_color = (255, 255, 255))
	...
	try:
		...
		if ":" in line:
		    ...
		    scoreslist.append([int(score), ' : '.join(name, score])
		
	finally:
	    fd.close()

	return sorted(scoreslist, key=lambda x: x[0])).reverse()
